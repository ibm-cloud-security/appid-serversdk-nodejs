...
function ApiStrategy(options) {
	...
	this.scope = options.scope || ApiStrategy.DEFAULT_SCOPE;
...
ApiStrategy.prototype.authenticate = function(req, options) {
	...
	options = options || {};
	// var requiredScope = ApiProtectionStrategy.DEFAULT_SCOPE;
	// requiredScope += (options.scope) ? " " + options.scope : "";
	var requiredScope = options.scope || ApiStrategy.DEFAULT_SCOPE;
	...
	// Validate header has exactly 2 or 3 components (Beader, access_token, [id_token])
	...
	var accessToken = tokenUtils.decodeAndValidate(accessTokenString);
	if (!accessToken){
	...
	if (authHeaderComponents.length == 3){
		var identityTokenString = authHeaderComponents[2];
		var identityToken = tokenUtils.decodeAndValidate(identityTokenString);
		if (!identityToken){
