...
module.exports = (function () {
	...
	function getPublicKeysEndpoint() {
                return (arguments.length > 1) ? arguments[1] + PUBLIC_KEYS_PATH : publicKeysEndpoint;
	}
	
	function _updatePublicKeys(publicKeysUrl) {
		var deferred = Q.defer();
		logger.debug("Getting public key from", publicKeysUrl);
		//publicKeysJson[publicKeysUrl] = 
		request({
			method: "GET",
			url: publicKeysUrl,
			json: true,
			...
			if (error || response.statusCode !== 200) {
				if (typeof publicKeysJson[publicKeysUrl] === 'undefined') {
					deferred.reject("Failed to update public keys.");
                                } else if (!(publicKeysJson[publicKeysUrl])) {
		...
		if (tokenKid) {
			...
			if (!publicKey) { // if not found, refresh public keys array
				eventEmitter.once("publicKeysUpdated", function(error) {
					var publicKeys = getPublicKeyByKid(tokenKid, localPublicKeysEndpoint);
					if (publicKeys) {
