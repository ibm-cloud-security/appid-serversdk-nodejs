...
module.exports = (function () {
    ...
    function checkAud(token, clientId) {
        ...
        if ((version === undefined || version <= 3) && token[constants.AUD] !== clientId) {
            logger.error(`Expected token ${constants.AUD} to be ["${clientId}"]`);
            return false;
        }
        if (version > 3) {
        	if (!Array.isArray(token[constants.AUD])) {
                logger.error(`Expected token ${constants.AUD} to be an array`);
                return false;
        	}
        	if (!token[constants.AUD].includes(clientId)) {
                logger.error(`Expected token ${constants.AUD} to include ${clientId} in its array`);
                return false;
            }
        }
        return true;
    ...
    function validateIss(token, serviceConfig) {
        return new Promise((resolve, reject) => {
			...
			serviceConfig.getIssuer()
				.then(issuer => {
					...
					reject(new Error("Unable to retrieve issuer"));
				});
        });
    }
	...
	function validateIssAndAud(token, serviceConfig) {
		return new Promise((resolve, reject) => {
			...
			// If configuration was initialized with user provided issuer.
            validateIss(token, serviceConfig)
				.then(result => {
					resolve(result);
				}).catch(error => {
					reject(error);
	...
	return {decodeAndValidate, decode, validateIss, validateIssAndAud, getRandomNumber};
}());
