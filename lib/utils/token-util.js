...
module.exports = (function () {
   ...
   * The scope prefixes are ignored (for a given scope on the access token we take only what is after the last slash).
   * We assume that the scopes on the access token are specific to the application - which is true for WebApp applications.
   * @param req
   * @param requiredScopes
   * @returns {boolean}
   */
  function hasScope(req, requiredScopes) {
	if (!requiredScopes) {
	  return true;
	}
  ...
  return {decodeAndValidate, decode, validateIssAndAud, getRandomNumber, hasScope};
}());
