 ...
 */
module.exports = (function TokenUtil() {
  ...
  function decodeAndValidate(tokenString, serverUrl) {
	...
	const allowExpiredTokens = process.env.APPID_ALLOW_EXPIRED_TOKENS || false;
	const token = decode(tokenString, true);
	const tokenHeader = token ? token.header : null;
	...
	publicKeyUtil.getPublicKeyPemByKid(tokenHeader.kid, serverUrl).then((publicKeyPem) => {
	  try {
  ...
  return {
	  decodeAndValidate, decode, validateIssAndAud, getRandomNumber
};
}());
