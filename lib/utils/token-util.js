...
module.exports = (function () {
  ...
  function validateExpiration(tokenPayload) {
	if (!tokenPayload || !tokenPayload[constants.EXP]) {
	  throw new Error('invalid token payload.');
	}
	if (Date.now().valueOf() >= tokenPayload[constants.EXP] * 1000) {
	  logger.error(`Expected token exp to be valid`);
  ...
  return {decodeAndValidate, decode, validateIssAndAud, getRandomNumber, validateExpiration};
}());
