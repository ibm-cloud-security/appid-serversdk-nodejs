...
module.exports = (function () {
  ...
  function hasScope(req, requiredScopes) {
	...
	const existingScopesArray = req.session[WebAppStrategy.AUTH_CONTEXT].accessTokenPayload.scope.split(" ");
	
	for (const requiredScope in requiredScopesArray) {
	  if (!existingScopesArray.includes(requiredScope)) {
		return false;
	  }
	}
	return true;
