...
module.exports = (function () {
  ...
  function hasScope(req, requiredScopes) {
	...
	const suppliedScopesArray = suppliedScopes.split(" ");
	
	const scopePrefix = req.session[WebAppStrategy.AUTH_CONTEXT].accessTokenPayload.appUri +'/'; // TODO: how do I get the app URI?
	for (const requiredScope of requiredScopesArray) {
	  if (!suppliedScopesArray.includes(scopePrefix + requiredScope)) {
		return false;
	  }
	}
	return true;
