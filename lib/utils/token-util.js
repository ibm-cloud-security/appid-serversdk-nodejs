...
module.exports = (function () {
    ...
    function checkVersion(token) {
        ...
        if (version === undefined) {
            logger.info(`token is older than v4`);
            return null;
		} else if (Number.isInteger(version) && version >= 4) {
    ...
    function checkAud(token, clientId) {
        ...
        if (version === false) {
            logger.error(`Token version is invalid: ${token[constants.VERSION]}\``);
            return false;
		} else if (version === null && token[constants.AUD] !== clientId) {
	...
	function checkIss(token, issuer) {
		logger.info(`token ${constants.ISS}: ${token[constants.ISS]}`);
		if (token[constants.ISS] !== issuer) {
			logger.error(`Expected token ${constants.ISS} to be ${issuer}`);
			return false;
		}
		return true;
	}
			...
			.then(()=>{
				...
				return updateWellKnownIssuer(serviceConfig).then(issuer => {
					if (checkIss(token, issuer)) {
						...
						throw new Error("Invalid issuer from well know endpoint:");
					}
				});
			});
