...
const loadConfig = (configName, requiredParams, options={}) => {
    ...
    if (discoveryEndpoint) {
        const response = getDiscovery(discoveryEndpoint);
...
const getDiscovery = async discoveryEndpoint => {
    return await request({headers: {'Accept': 'application/json'}, url: discoveryEndpoint}, function (error, response, body) {
        if (error || !response.statusCode === 200) {
            throw Error(`GET discovery request failed. Status: ${response.statusCode}. Error: ${error.message}`);
        }
