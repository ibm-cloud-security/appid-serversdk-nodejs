 ...
 * @return {ServiceConfig} the configuration
 */
const loadConfig = (configName, requiredParams, options={}) => {
    ...
    if (discoveryEndpoint) {
        getDiscovery(discoveryEndpoint)
            .then(response => {
                serviceConfig[constants.OAUTH_SERVER_URL] = response.authorization_endpoint;
                serviceConfig[constants.USER_PROFILE_SERVER_URL] = response.profiles_endpoint;

                requiredParams.map(checkRequiredParams(serviceConfig, configName));
                return {
                    getConfig,
                    getTenantId,
                    getClientId,
...
const checkRequiredParams = (serviceConfig, configName) => {
    return param => {
