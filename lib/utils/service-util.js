...
const loadConfig = (configName, requiredParams, options={}) => {
    return new Promise(async (resolve, reject) => {
        ...
        const configMethods = {
            ...
            setIssuer: (value) => serviceConfig[constants.APPID_ISSUER] = value
        };

        if (discoveryEndpoint) {
            getDiscovery(discoveryEndpoint)
                .then(response => {
                    const oauthServerUrl = response.authorization_endpoint.slice(0, response.authorization_endpoint.indexOf("/authorization"));
...
const paramChecker = (serviceConfig, configName) => {
    return param => {
