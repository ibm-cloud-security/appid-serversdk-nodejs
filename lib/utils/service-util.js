...
const getInformation = (discoveryEndpoint, serviceConfig) => {
	...
	if (!isUpdateRequestPending) {
		...
		getDiscovery(discoveryEndpoint)
			.then(response => {
				...
				serviceConfig[constants.OAUTH_SERVER_URL] = response[constants.APPID_DISCOVERY_AUTHORIZATION]
				...
				const error = "getDiscoveryUpdated error: " + e;
				logger.error(error);
...
function loadConfig(configName, requiredParams, options={}) {
	...
	requiredParams = requiredParams.filter(param => param !== constants.APPID_ISSUER &&
													param !== constants.USER_PROFILE_SERVER_URL);

	if (discoveryEndpoint) {
		requiredParams
			.filter(param => param !== constants.OAUTH_SERVER_URL)
			.map(paramChecker(serviceConfig, configName));
