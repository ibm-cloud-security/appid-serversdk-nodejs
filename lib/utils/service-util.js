...
function loadConfig(configName, requiredParams, options) {
	...
	if (options[constants.APPID_SERVICE_ENDPOINT]) {
		if (!options[constants.APPID_SERVICE_VERSION]) {
			...
			serviceConfig[constants.OAUTH_SERVER_URL] = `${options[constants.APPID_SERVICE_ENDPOINT]}/oauth/${options[constants.APPID_SERVICE_VERSION]}/${options[constants.APPID_TENANT_ID]}`;
		}
