...
function loadConfig(configName, requiredParams, options) {
	...
	let serviceConfig = {};
	if (options[constants.APPID_SERVICE_ENDPOINT]) {
		if (!options[constants.APPID_SERVICE_VERSION]) {
			throw new Error("Failed to initialize APIStrategy. Missing version parameter");
		} else if (!options[constants.APPID_TENANT_ID]) {
			throw new Error("Failed to initialize APIStrategy. Missing tenantId parameter");
		} else {
			serviceConfig[constants.OAUTH_SERVER_URL] = `${options[constants.APPID_SERVICE_ENDPOINT]}/oauth/v${options[constants.APPID_SERVICE_VERSION]}/${options[constants.APPID_TENANT_ID]}`;
		}
	...
	serviceConfig[constants.APPID_ISSUER] = serviceConfig[constants.APPID_ISSUER] || options[constants.APPID_ISSUER];
