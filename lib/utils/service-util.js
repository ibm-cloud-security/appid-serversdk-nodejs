        ...
        if (discoveryEndpoint) {
            ...
            if (serviceEndpoint) {
                if (!serviceVersion || !Number.isInteger(Number.parseInt(serviceVersion))) {
                    reject(new Error("Failed to initialize APIStrategy. Missing version parameter, should be an integer."));
                } else if (!serviceTenantID) {
                    reject(new Error("Failed to initialize APIStrategy. Missing tenantId parameter"));
                } else {
