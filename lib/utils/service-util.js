...
function loadConfig(configName, requiredParams, options={}) {
	...
	if (serviceEndpoint) {
		if (!serviceVersion || !Number.isInteger(Number.parseInt(serviceVersion))) {
            ...
            getDiscovery(serviceEndpoint, serviceTenantID, serviceVersion)
				.then(response => {
					...
					throw Error(`Failed to initialize ${configName}. Missing urls from discovery endpoint.`);
				})
		}
	} else {
...
const getDiscovery = (serviceEndpoint, serviceTenantID, serviceVersion) => {
	const req = {
        headers: {'Accept': 'application/json'},
        url: `${serviceEndpoint}/oauth/${serviceVersion}/${serviceTenantID}/.well-known/openid-configuration`
    };
