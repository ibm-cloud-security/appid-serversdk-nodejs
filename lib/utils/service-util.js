 ...
 * @property {function():string} getVersion - returns the version
...
function loadConfig(configName, requiredParams, options={}) {
    ...
    let serviceVersion = findParam(constants.APPID_SERVICE_VERSION);
	let serviceConfig = {};

	if (discoveryEndpoint) {
        request({headers: {'Accept': 'application/json'}, url: discoveryEndpoint}, (error, response, body) => {
            if (error) {
                logger.error('GET discovery request error: ' + error.message);
                throw Error(`Failed to retrieve information from GET discovery endpoint`);
            } else {
                if (response.statusCode === 200) {
                    serviceConfig[constants.OAUTH_SERVER_URL] = body.authorization_endpoint;
                    serviceConfig[constants.USER_PROFILE_SERVER_URL] = body.profiles_endpoint;
                    serviceVersion = getVersionFromDiscoveryEndpoint(discoveryEndpoint, serviceTenantID);
                } else {
                    logger.error('GET discovery response not 200, got status:' + response.statusCode);
                    throw Error(`Failed to retrieve information from GET discovery endpoint`);
                }
            }
        });
    } else if (serviceEndpoint) {
    ...
    serviceConfig[constants.APPID_VERSION] = serviceVersion;
    ...
    const getVersion = () => serviceConfig[constants.APPID_VERSION];
    const getUserProfile = () => serviceConfig[constants.APPID_VERSION];
	//setters
	const setIssuer = (value) => serviceConfig[constants.USER_PROFILE_SERVER_URL] = value;
...
const getVersionFromDiscoveryEndpoint = (discoveryEndpoint, serviceTenantID) => {
    const tenantIndex = discoveryEndpoint.indexOf(serviceTenantID);
    const substring = discoveryEndpoint.slice(0, tenantIndex - 1);
    const startingVersionIndex = substring.lastIndexOf("/") + 2;
    return substring.slice(startingVersionIndex);
};
