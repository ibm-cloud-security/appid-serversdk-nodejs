...
function loadConfig(configName, requiredParams, options={}) {
	...
	const serviceConfig = {};
	if (serviceEndpoint) {
		if (!serviceVersion || !Number.isInteger(Number.parseInt(serviceVersion, 10))) {
	...
	serviceConfig[constants.APP_URI] = findParam(constants.APP_URI);
	...
	if (!serviceConfig[constants.REDIRECT_URI]) {
		...
		if (vcapApplication) {
			...
			const redirectUri = `https://${vcapApplication["application_uris"][0]}/ibm/bluemix/appid/callback`;
			serviceConfig[constants.REDIRECT_URI] = redirectUri;
		}
	}
	...
	const getAppUri = () => serviceConfig[constants.APP_URI];
	...
	const setIssuer = (value) => {
		serviceConfig[constants.APPID_ISSUER] = value;
	};
