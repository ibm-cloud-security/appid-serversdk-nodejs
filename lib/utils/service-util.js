...
const loadConfig = (configName, requiredParams, options={}) => {
    return new Promise((resolve, reject) => {
        ...
        const configMethods = {
            ...
            getIssuer: () => serviceConfig[constants.APPID_ISSUER],
            getUserProfile: () => serviceConfig[constants.USER_PROFILE_SERVER_URL]
        };
        ...
        if (discoveryEndpoint) {
            getDiscovery(discoveryEndpoint)
                .then(response => {
                    ...
                    serviceConfig[constants.USER_PROFILE_SERVER_URL] = response[constants.APPID_DISCOVERY_AUTHORIZATION];
                    ...
                    resolve({
                        ...configMethods,
                        getOAuthServerUrl: getDiscoveryIfUndefined(discoveryEndpoint, configMethods.getOAuthServerUrl(), constants.OAUTH_SERVER_URL),
                        getIssuer: getDiscoveryIfUndefined(discoveryEndpoint, configMethods.getIssuer(), constants.APPID_ISSUER),
                        getUserProfile: getDiscoveryIfUndefined(discoveryEndpoint, configMethods.getUserProfile(), constants.USER_PROFILE_SERVER_URL)
                    });
                }).catch(error => {
                    throw Error(error);
                });
    ...
    return new Promise((resolve, reject) => {
        request({headers: {'Accept': 'application/json'}, url: discoveryEndpoint}, function (error, response, body) {
            if (error || !response.statusCode === 200) {
                reject(`GET discovery request failed. Status: ${response.statusCode}. Error: ${error.message}`);
            }
...
const getDiscoveryIfUndefined = (discoveryEndpoint, getValue, constant) => {
    if (getValue) {
        return getValue;
    }
    getDiscovery(discoveryEndpoint)
