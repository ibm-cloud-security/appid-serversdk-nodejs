...
const FORGOT_PASSWORD_RESULT = "forogot password result";
...
const MGMT_CHANGE_PASSWORD_PATH = MGMT_FORGOT_PASSWORD_PATH + "/change_password";
 ...
 * @param {string} secretKey, the secretKey to use to get the stored result for sign up confirmation.
 * @param {string=} iamToken, optional, if passed request to the server will use this token.
 * @return 200 with the json result, if secretKey is valid, else reject with the error.
 */
SelfServiceManager.prototype.getSignUpConfirmationResult = function (secretKey, iamToken) {
	const deferred = Q.defer();
	let url = this.managementUrl + MGMT_SIGN_UP_RESULT_PATH;
	_getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then(function (iamToken) {
		handleRequest(iamToken, POST, url, {secretKey: secretKey} , {}, SIGN_UP_RESULT, deferred);
	}).catch(deferred.reject);
	return deferred.promise;
};
 ...
 * @param {string} secretKey, the secretKey to use to get the stored result for forgot password confirmation.
 * @param {string=} iamToken, optional, if passed request to the server will use this token.
 * @return 200 with the json result, if secretKey is valid, else reject with the error.
 */
SelfServiceManager.prototype.getForgotPasswordConfirmationResult = function (secretKey, iamToken) {
	const deferred = Q.defer();
	let url = this.managementUrl + MGMT_FORGOT_PASSWORD_RESULT_PATH;
	_getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then(function (iamToken) {
		handleRequest(iamToken, POST, url, {secretKey: secretKey} , {}, FORGOT_PASSWORD_RESULT, deferred);
	}).catch(deferred.reject);
	return deferred.promise;
};
 ...
 * @param {string} uuid, the secretKey to use to get the forgot password stored result.
