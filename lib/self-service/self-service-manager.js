...
function SelfServiceManager(options={}) {
	...
	// OAUTH_V4 is used to construct management URL -- let's get the management URL differently
	// Let's see where this class is used and how it's initialized
	if (!this.managementUrl) {
		...
		if (!oauthServerUrl || oauthServerUrl.indexOf(OAUTH_V4) === -1) {
			logger.error(initError);
			logger.error(initErrorMsg);
			throw new Error(initError);
		}
		...
		let serverUrl = oauthServerUrl.split(OAUTH_V4)[0];
