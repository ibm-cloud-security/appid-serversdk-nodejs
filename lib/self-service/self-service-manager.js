...
let _handleRequest = function _handleRequestFunc(iamToken, method, url, body, queryObject, action, deferred) {
  const reqOptions = {
    url,
    method,
    qs: queryObject,
  ...
  request(reqOptions, (err, response, responseBody) => {
    if (!err && response.statusCode >= 200 && response.statusCode < 300) {
      ...
      if (err) {
        ...
        error.message = (responseBody && (responseBody.detail || responseBody.message || responseBody.error)) ||
          ((!_.isString(responseBody)) ? JSON.stringify(responseBody) : responseBody);
      }
...
// eslint-disable-next-line prefer-const
let _getIAMToken = function _getIAMTokenFunc(iamToken, iamApiKey, iamTokenUrl) {
  if (iamToken && _.isString(iamToken)) {
    return Promise.resolve(iamToken);
  }
  if (!iamApiKey) {
    return Promise.reject(new Error(
      "You must pass 'iamToken' to self-service-manager APIs or specify 'iamApiKey' in selfServiceManager init options."
    ));
  }
  const reqOptions = {
    url: iamTokenUrl,
  ...
  return new Promise(((resolve, reject) => {
    request(reqOptions, (error, response, body) => {
      if (error) {
        ...
        const IAMAccessToken = JSON.parse(body)["access_token"];
 ...
 * if managementUrl is not specified it will be taken from the App ID service
 * binding json if it exist or constructed using the oauthServerUrl.
 *
...
function SelfServiceManager(options = {}) {
  ...
  const tenantId = options[TENANT_ID] || vcapServiceCredentials[TENANT_ID];
  ...
  if (!this.managementUrl) {
    ...
    const oauthServerUrl = options[OAUTH_SERVER_URL] || options['oAuthServerUrl'] || vcapServiceCredentials[OAUTH_SERVER_URL];
    ...
    const serverUrl = oauthServerUrl.split(OAUTH_V3)[0];
    const serverDomain = serverUrl.split(APPID_AUTH);
    if (serverDomain[1]) {
 ...
 * @param {Object} userData must include at least one email address and 'password' attribute (as defined in the specification).
 * @param {string} [language='en'] the user language code.
 * @param {string=} iamToken if passed request to the server will use this token.
 * @return {Object} The created user SCIM.
 */
SelfServiceManager.prototype.signUp = function signUp(userData, language = 'en', iamToken) {
  const deferred = Q.defer();
  const url = this.managementUrl + MGMT_SIGN_UP_PATH;
  _getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then((token) => {
 ...
 */
SelfServiceManager.prototype.forgotPassword = function forgotPassword(email, language = 'en', iamToken) {
  const deferred = Q.defer();
  const url = this.managementUrl + MGMT_FORGOT_PASSWORD_PATH;
  _getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then((token) => {
 ...
 * @param {string} [iamToken] if passed request to the server will use this token.
 *
 * @return {Promise} resolve if notification sent successfully, else reject with the error.
 */
SelfServiceManager.prototype.resendNotification = function resendNotification(uuid, templateName, language = 'en', iamToken) {
  const deferred = Q.defer();
  const url = this.managementUrl + MGMT_RESEND_PATH + templateName;
  _getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then((token) => {
    _handleRequest(token, POST, url, {
      uuid
    }, {
 ...
 * @param {string} [iamToken] if passed request to the server will use this token.
 *
 * @return {Promise} resolve with the json result, if context is valid, else reject with the error.
 */
SelfServiceManager.prototype.getSignUpConfirmationResult = function getSignUpConfirmationResult(context, iamToken) {
  const deferred = Q.defer();
  const url = this.managementUrl + MGMT_SIGN_UP_RESULT_PATH;
  _getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then((token) => {
    _handleRequest(token, POST, url, {
      context
 ...
 * @return {Promise} resolve with the json result, if context is valid, else reject with the error.
 */
SelfServiceManager.prototype.getForgotPasswordConfirmationResult =
  function getForgotPasswordConfirmationResult(context, iamToken) {
    const deferred = Q.defer();
    const url = this.managementUrl + MGMT_FORGOT_PASSWORD_RESULT_PATH;
    _getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then((token) => {
      _handleRequest(token, POST, url, {
        context
 ...
 * @return {Object} The user SCIM profile.
 */
SelfServiceManager.prototype.setUserNewPassword =
  function setUserNewPassword(uuid, newPassword, language = 'en', changedIpAddress = null, iamToken) {
    const deferred = Q.defer();
    const url = this.managementUrl + MGMT_CHANGE_PASSWORD_PATH;
    _getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then((token) => {
      const replacePasswordBody = {
        uuid,
        newPassword
      };
      if (changedIpAddress) {
 ...
 * @return {Object} The user SCIM profile.
 */
SelfServiceManager.prototype.getUserDetails = function getUserDetails(uuid, iamToken) {
  const deferred = Q.defer();
  const url = this.managementUrl + MGMT_USERS_PATH + uuid;
  _getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then((token) => {
 ...
 * @return {Object} The user SCIM profile.
 */
SelfServiceManager.prototype.updateUserDetails = function updateUserDetails(uuid, userData, iamToken) {
  const deferred = Q.defer();
  const url = this.managementUrl + MGMT_USERS_PATH + uuid;
  _getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then((token) => {
