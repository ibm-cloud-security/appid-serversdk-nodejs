 ...
 * @return {Object} The user SCIM profile.
 */
SelfServiceManager.prototype.setUserNewPassword = function (uuid, newPassword, changedIpAddress=null, iamToken) {
	const deferred = Q.defer();
	...
	_getIAMToken(iamToken, this.iamApiKey, this.iamTokenUrl).then(function (token) {
		...
		_handleRequest(token, POST, url, replacePasswordBody , {}, CHANGE_USER_PASSWORD, deferred);
	}).catch(deferred.reject);
	return deferred.promise;
};
	...
	request(reqOptions, function (err, response, responseBody) {
		if(!err && response.statusCode >= 200 && response.statusCode < 300) {
			...
			if (err) {
				...
				error.message = responseBody && (responseBody.detail || responseBody.message || responseBody.error) || responseBody;
			}
