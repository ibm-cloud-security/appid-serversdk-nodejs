...
UserManager.prototype.init = function (options) {
	options = options || {};
...
UserManager.prototype.setAttribute = function (accessToken, attributeName, attributeValue) {
...
UserManager.prototype.getAttribute = function (accessToken, attributeName) {
	const deferred = Q.defer();
...
UserManager.prototype.deleteAttribute = function (accessToken, attributeName) {
	const deferred = Q.defer();
	if (!_.isString(accessToken) || !_.isString(attributeName)) {
...
UserManager.prototype.getAllAttributes = function (accessToken) {
	const deferred = Q.defer();
 ...
 */
UserManager.prototype.getUserInfo = function (accessToken, identityToken) {
	const deferred = Q.defer();
	...
	let serverUrl = this.oauthServerUrl + USERINFO_ENDPOINT;
	...
	internalDeferred.promise
		.then(function(profile) {
			...
			if (identityToken) {
				...
				if (!expectedSub) {
					return deferred.reject("Invalid identity token")
				}
