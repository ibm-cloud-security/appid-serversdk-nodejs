...
UserProfileManager.prototype.init = function (options) { // eslint-disable-line complexity
	...
	if (options[constants.APPID_SERVICE_ENDPOINT]) {
		if (!options[constants.APPID_SERVICE_VERSION]) {
			...
			this.userProfilesServerUrl = options[constants.APPID_SERVICE_ENDPOINT];
			this.oauthServerUrl = `${options[constants.APPID_SERVICE_ENDPOINT]}/oauth/v${options[constants.APPID_SERVICE_VERSION]}/${options[constants.APPID_TENANT_ID]}`;
		}
