...
function UserProfileManager(options) {}
...
UserProfileManager.prototype.init = function (options) { // eslint-disable-line complexity
    ...
    this.serviceConfig = serviceUtils.loadConfig("user profile config", [constants.OAUTH_SERVER_URL, constants.USER_PROFILE_SERVER_URL], options)
};
...
UserProfileManager.prototype.setAttribute = function (accessToken, attributeName, attributeValue) {
    ...
    console.log(this);
    this.serviceConfig.getUserProfile()
		.then(userProfilesServerUrl => {
			console.log("Returned profile server url")
			console.log(userProfilesServerUrl);
            const profilesUrl = userProfilesServerUrl + ATTRIBUTES_ENDPOINT + "/" + attributeName;
...
UserProfileManager.prototype.getAttribute = function (accessToken, attributeName) {
	const deferred = Q.defer();
	const self = this;
    // eventEmitter.once("UserProfileServiceConfigLoaded", function() {
        ...
        var profilesUrl = self.userProfilesServerUrl + ATTRIBUTES_ENDPOINT + "/" + attributeName;
...
UserProfileManager.prototype.getAllAttributes = function (accessToken) {
    ...
    // eventEmitter.once("UserProfileServiceConfigLoaded", function() {
        ...
        var profilesUrl = self.userProfilesServerUrl + ATTRIBUTES_ENDPOINT;
...
UserProfileManager.prototype.getUserInfo = function (accessToken, identityToken) {
    ...
    // eventEmitter.once("UserProfileServiceConfigLoaded", function() {
        ...
        const serverUrl = self.oauthServerUrl + USERINFO_ENDPOINT;
