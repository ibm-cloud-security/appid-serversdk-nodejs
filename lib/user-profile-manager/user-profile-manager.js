	...
	serviceUtils.loadConfig("user profile config", [constants.OAUTH_SERVER_URL, constants.USER_PROFILE_SERVER_URL], options)
		.then(config => {
			Promise.all([
				...
				this.oauthServerUrl = urls[0] || options[constants.OAUTH_SERVER_URL] ||
					vcapServiceCredentials[constants.OAUTH_SERVER_URL];
				this.userProfilesServerUrl = urls[1] || options[constants.USER_PROFILE_SERVER_URL] ||
					vcapServiceCredentials[constants.USER_PROFILE_SERVER_URL];

                eventEmitter.emit("UserProfileServiceConfigLoaded");
...
UserProfileManager.prototype.setAttribute = function (accessToken, attributeName, attributeValue) {
    ...
    eventEmitter.once("UserProfileServiceConfigLoaded", function() {
        ...
        console.log("Profile URL when needed");
        console.log(this.userProfilesServerUrl);
