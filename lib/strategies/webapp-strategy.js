...
function handleRopFlow(options, req, strategy) {
	...
	retrieveTokens(formData, strategy).then(function (appIdAuthContext) {
		completion(appIdAuthContext, strategy, req, options);
	}).catch(function (err) {
		strategy.fail(err);
	});
}
...
function completion(appIdAuthContext, strategy, req, options) {
	...
	request({
		...
		if (err) {
			...
			try {
				...
				let loginError = body && body.error_description;
				deferred.reject(new Error(loginError || "Failed to obtain tokens"));
			}catch (e) {
