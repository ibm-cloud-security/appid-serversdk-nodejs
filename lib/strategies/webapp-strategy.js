...
function handleCallback(req, options, strategy) {
	...
	let stateParameter = strategy.serviceConfig.getStateParameter();
	if (!stateParameter.anonymousLogin && typeof req.query.state === 'undefined') {
...
function generateAuthorizationUrl(options, req, strategy) {
	...
	if (options.allowAnonymousLogin === true) {
		strategy.serviceConfig.generateStateParameter(true);
		authUrl += "&idp=appid_anon";
	} else {
		authUrl += "&state=" + strategy.serviceConfig.generateStateParameter(false).state;
	}
