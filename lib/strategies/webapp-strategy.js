...
function retrieveTokens(formData, strategy) {
	...
	request({
		...
		if (err) {
            ...
            TokenUtil.decodeAndValidate(accessTokenString, serviceConfig).then(function (accessToken) {
                if (!accessToken) {
                    logger.warn("Invalid access_token");
                    deferred.reject(new Error("Invalid access_token"));
                    return;
                }
                if(TokenUtil.validateToken(accessToken, serviceConfig)) {
                    appIdAuthorizationContext.accessToken = accessTokenString;
                    appIdAuthorizationContext.accessTokenPayload = accessToken;
                    TokenUtil.decodeAndValidate(identityTokenString,serviceConfig).then(function (identityToken) {
                    	if (identityToken && TokenUtil.validateToken(identityToken,serviceConfig)) {
                    		appIdAuthorizationContext.identityToken = identityTokenString;
                    		appIdAuthorizationContext.identityTokenPayload = identityToken;
		                    logger.debug("authentication success");
		                    deferred.resolve(appIdAuthorizationContext);
                    	} else {
		                    deferred.reject(new Error("identity_token validation failed"));
	                    }
                    }).catch(function (err) {
                    	logger.warn("Invalid identity_token");
                    	deferred.reject(new Error("Invalid identity_token"));
                    });
                } else {
	                deferred.reject(new Error("access_token validation failed"));
                }
            }).catch(function (err) {
