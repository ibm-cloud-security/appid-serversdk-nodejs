...
function isAuthenticated(req, options = {}) {
  ...
  const shouldValidateExpiration = options.hasOwnProperty("shouldValidateTokenExpiration") ?
	options.shouldValidateTokenExpiration : false;
  
  if (shouldValidateExpiration) {
    try {
      TokenUtil.validateExpiration(req.session[WebAppStrategy.AUTH_CONTEXT].accessTokenPayload);
    } catch (e) {
      // token expired
      return false;
    }
