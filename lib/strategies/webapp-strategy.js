...
/**
 * If you use the sample application , add to the app.js an express routing such as
 * app.get("/logoutSSO", function(req, res, next) {
 *	res.clearCookie("refreshToken");
 *  webAppStrategy.logoutSSO(req,res, { successRedirect: "http://localhost:3000/myspecialSSOLogoutPage" });
 * });
 * @param req the http request
 * @param res - the response object (will be used to redirect)
 * @param options - need to contain successRedirect.
 */
WebAppStrategy.prototype.logoutSSO = function (req, res, options) {
	let redirectUri = {};
	if (options.successRedirect) {
		redirectUri = options.successRedirect;
	}
	let oauthServerUrl = this.serviceConfig.getOAuthServerUrl();
	let clientId = this.serviceConfig.getClientId();
	let url = `${oauthServerUrl}/cloud_directory/sso/logout?client_id=${clientId}&callback=${redirectUri}`;
