...
function WebAppStrategy(options) {
	...
	const { CLIENT_ID, TENANT_ID, SECRET, OAUTH_SERVER_URL, REDIRECT_URI } = constants;
...
function redirectForAuthorization(req, options, strategy) {
	...
	var stateParameter = { anonymousLogin: false, state: TokenUtil.getRandomNumber() };
	if (options.allowAnonymousLogin === true) {
		stateParameter.anonymousLogin = true;
	}
...
function handleCallback(req, options, strategy) {
	...
	let stateParameter = req.session[WebAppStrategy.STATE_PARAMETER];
	if (typeof stateParameter === "undefined" || (!stateParameter.anonymousLogin && typeof req.query.state === "undefined")) {
		strategy.fail(new Error("Missing state parameter"));
	} else if (!stateParameter.anonymousLogin && stateParameter.state !== decodeURIComponent(req.query.state)) {
