...
WebAppStrategy.logout = function logout(req) {
  finishLogout(req);
};
...
function generateForgotPasswordUrl(req, strategy) {
  ...
  const forgotPasswordUrl = encodeURI(`${forgotPasswordEndpoint
...
function generateChangePasswordUrl(userId, req, strategy) {
  ...
  const redirectUri = serviceConfig.getRedirectUri();
  const changePasswordUrl = encodeURI(`${changePasswordEndpoint
		}?client_id=${clientId
...
function generateChangeDetailsUrl(code, req, strategy) {
  ...
  const redirectUri = serviceConfig.getRedirectUri();
  const changeDetailsUrl = encodeURI(`${changeDetailsEndpoint
		}?client_id=${clientId
		}&redirect_uri=${redirectUri
		}&code=${code}`);
...
function generateAuthorizationUrl(options, req, strategy) {
  ...
  let authUrl = encodeURI(`${authorizationEndpoint
		}?client_id=${clientId
...
function handleChangePassword(req, options, strategy) {
  ...
  const userId = appIdAuthContext.identityTokenPayload["identities"][0]["id"];
  const changePasswordUrl = generateChangePasswordUrl(userId, req, strategy);
...
function handleForgotPassword(req, options, strategy) {
  ...
  const forgotPasswordUrl = generateForgotPasswordUrl(req, strategy);
...
function handleChangeDetails(req, options, strategy) {
  ...
  const generateCodeUrl = strategy.serviceConfig.getOAuthServerUrl() + GENERATE_CODE_PATH;
  request({
    url: generateCodeUrl,
    auth: {
    ...
    if (!error) {
      if (response.statusCode === 200) {
        const code = body;
        const changeDetailsUrl = generateChangeDetailsUrl(code, req, strategy);
    ...
    request({
      ...
      if (response.statusCode !== 200) {
        try {
          ...
          const body = strBody && JSON.parse(strBody);
          ...
          const loginError = new Error(loginErrMsg);
        ...
        // Parse access_token
        const appIdAuthorizationContext = {
          refreshToken
        };

        return Promise.all([
          TokenUtil.decodeAndValidate(accessTokenString, publicKeysEndpoint),
          TokenUtil.decodeAndValidate(identityTokenString, publicKeysEndpoint)
        ]).then((tokens) => {
          ...
          return TokenUtil.validateIssAndAud(accessToken, serviceConfig)
            ...
            .then(() => {
              ...
              appIdAuthorizationContext.accessTokenPayload = accessToken;
              appIdAuthorizationContext.identityToken = identityTokenString;
              appIdAuthorizationContext.identityTokenPayload = idToken;
          ...
          return reject(new Error(`Authentication failed : ${error.message}`));
        });
      }
    });
  });
...
function getTokens(req, options, strategy) {
  ...
  retrieveTokens(formData, strategy).then((appIdAuthContext) => {
...
function handleRopFlow(options, req, strategy) {
  ...
  retrieveTokens(formData, strategy).then((subAppIdAuthContext) => {
    // Save authorization context to HTTP session
    req.session[WebAppStrategy.AUTH_CONTEXT] = subAppIdAuthContext;
    logger.debug("completeRopFlow :: success", options);
    strategy.success(subAppIdAuthContext.identityTokenPayload || null);
  }).catch(strategy.fail);
}
...
function redirectForAuthorization(req, options, strategy) {
  options.allowCreateNewAnonymousUser = Object.prototype.hasOwnProperty.call(options, "allowCreateNewAnonymousUser") ?
    options.allowCreateNewAnonymousUser : true;
  options.failureRedirect = options.failureRedirect || "/";
  let authUrl = generateAuthorizationUrl(options, req, strategy);
  ...
  const stateParameter = {
    state: TokenUtil.getRandomNumber()
  };
...
function handleAuthorization(req, options = {}, strategy) {
  ...
  return redirectForAuthorization(req, options, strategy);
}
...
WebAppStrategy.prototype.logout = function logout(req) {
  logAction(req, this.serviceConfig.getOAuthServerUrl(), LOGOUT_ACTIVITY);
  finishLogout(req);
};

WebAppStrategy.prototype.setPreferredLocale = function setPreferredLocale(req, language) {
...
WebAppStrategy.prototype.getPreferredLocale = function getPreferredLocale(req) {
...
WebAppStrategy.prototype.authenticate = function authenticate(req, options = {}) {
  if (options.successRedirect) {
  ...
  if (req.query && req.query.error) {
    ...
    const returnError = req.query.error_description || req.query.error;
  ...
  if (req.query && req.query.flow && (req.query.flow === WebAppStrategy.SIGN_UP ||
...
WebAppStrategy.prototype.refreshTokens = function refreshTokensProto(req, refreshToken) {
  return refreshTokens(refreshToken, this).then((appIdAuthContext) => {
    // Save authorization context to HTTP session
    req.session[WebAppStrategy.AUTH_CONTEXT] = appIdAuthContext;
  });
};
 ...
 * @param {Response} res - the HTTP response object (will be used to redirect)
 * @param {Object} options - need to contain at least the redirect_uri. all_sessions can logout all the users sessions at once.
 *
 * @returns {undefined}
 */
WebAppStrategy.prototype.logoutSSO = function logoutSSO(req, res, options = {}) {
  options.client_id = this.serviceConfig.getClientId();
  const queryParams = Object.keys(options).map((e) => `${encodeURIComponent(e)}=${encodeURIComponent(options[e])}`).join('&');
  const oauthServerUrl = this.serviceConfig.getOAuthServerUrl();
  const url = `${oauthServerUrl}/cloud_directory/sso/logout?${queryParams}`;
 ...
 * @param {Request} req The request containing the App ID token.
 * @param {string} requiredScopes Scope names (not fullName) separated by spaces. For example: 'write read update'.
 *
 * @returns {boolean} Whether or not the token has the required scopes
 */
WebAppStrategy.hasScope = function hasScope(req, requiredScopes) {
  ...
  for (let i = 0; i < requiredScopesArray.length; i++) {
    const requiredScope = requiredScopesArray[i];
