...
function WebAppStrategy(options = {},verify) {
...
function handleCallback(req, options = {}, strategy) {
	...
	} else if (stateParameter.state !== req.query.state) {
...
function getTokens(req, options, strategy) {
	...
	function cb(err, IDToken, info) {
		if (err) { return strategy.error(err); }
		if (!IDToken) { return strategy.fail(info); }
		return strategy.success(null, IDToken || null, info);
	}
	retrieveTokens(formData, strategy).then(function (appIdAuthContext) {
		// Save authorization context to HTTP session
		req.session[WebAppStrategy.AUTH_CONTEXT] = appIdAuthContext;
		let accessToken = appIdAuthContext.accessTokenPayload;
		let IDToken = appIdAuthContext.identityTokenPayload;
		let refreshToken = appIdAuthContext.refreshToken;
		delete req.session[WebAppStrategy.STATE_PARAMETER];
		logger.debug("completeAuthorizationFlow :: success", options);
		strategy._verify(accessToken, IDToken, refreshToken, cb);
	}).catch(strategy.fail);
}
