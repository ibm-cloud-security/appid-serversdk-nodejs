...
WebAppStrategy.ensureAuthenticated = function(notAuthenticatedRedirect, allowAnonymous){
...
function handleAuthorization(req, options, strategy){
	...
	if (!appIdAuthContext && options.allowCreateNewAnonymousUser !== true){
	...
	logger.debug("handleAuthorization :: starting anonymous login");
	request({
		url: authUrl,
		followRedirect: false
	}, function(err, response, body){
		if (err || response.statusCode !== 302){
			logger.error("Anonymous login failed", err, response.statusCode, body);
			strategy.fail(new Error("Anonymous login failed"));
		} else {
			var redirectLocation = response.headers.location;
			var code = redirectLocation.split("?code=")[1];
			retrieveTokens(options, strategy, code).then(function(newAppIdAuthContext){
				completeAuthorizationFlow(req, options, strategy, newAppIdAuthContext);
			}).catch(strategy.fail);
		}
	});
}
...
function handleCallback(req, options, strategy) {
	...
	retrieveTokens(options, strategy, code).then(function(appIdAuthContext){
		completeAuthorizationFlow(req, options, strategy, appIdAuthContext);
	}).catch(strategy.fail);
}
...
function completeAuthorizationFlow(req, options, strategy, appIdAuthContext){
