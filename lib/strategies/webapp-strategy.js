	...
	request({
		...
		if (err) {
			...
			Promise.all([TokenUtil.decodeAndValidate(accessTokenString, serviceConfig),
				TokenUtil.decodeAndValidate(identityTokenString,serviceConfig)]).then(function(data){
				if (data.length < 2 || !data[0] || !data[1]) {
					deferred.reject(new Error("Invalid access/id token"));
					return;
				}

				if (!TokenUtil.validateIssAndAud(data[0], serviceConfig)) {
					deferred.reject(new Error("Access token validation failed"));
					return;
				}

				if (!TokenUtil.validateIssAndAud(data[1], serviceConfig)) {
					deferred.reject(new Error("Id token validation failed"));
					return;
				}
				appIdAuthorizationContext.accessToken = accessTokenString;
				appIdAuthorizationContext.accessTokenPayload = data[0];
				appIdAuthorizationContext.identityToken = identityTokenString;
				appIdAuthorizationContext.identityTokenPayload = data[1];
				logger.debug("authentication success");
				deferred.resolve(appIdAuthorizationContext);
			}).catch(function (err) {
				logger.debug("Authentication failed : " + err.message);
				deferred.reject(new Error("Authentication failed : " + err.message));
			});
		}
	});
