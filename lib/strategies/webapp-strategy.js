...
WebAppStrategy.prototype.authenticate = function (req, options) {
	...
	} else if (options.action === "refreshTokens") {
		var strategy = this;
		if (req.isAuthenticated() && !options.forceLogin) {
			return strategy.success(req.session[WebAppStrategy.AUTH_CONTEXT].identityTokenPayload);
		}
		return obtainTokensWithRefreshToken(req, options, strategy).catch(function() {
			strategy.pass();
		});
	} else {
...
function handleAuthorization(req, options, strategy) {
	...
	/* Trying to get a refresh-token from the user's function getRefreshToken.
	 * If either
	 * - The function is not implemented
	 * - No refresh token returned
	 * - An invalid one returned (server's validation failed),
	 * Continue with the original authorization flow
	*/
	if (typeof(strategy.getRefreshToken) === "function" && !options.forceLogin) {
		obtainTokensWithRefreshToken(req, options, strategy).catch(function() {
			logger.debug("handleAuthorization:: Continue to authorization");
...
function obtainTokensWithRefreshToken(req, options, strategy) {
	return Q.resolve().then(function() {
		return strategy.getRefreshToken(req);
	}).then(function(refreshToken) {
		return refreshTokens(refreshToken, strategy);
	}).then(function (appIdAuthContext) {
		// Save authorization context to HTTP session
		req.session[WebAppStrategy.AUTH_CONTEXT] = appIdAuthContext;
		strategy.success(appIdAuthContext.identityTokenPayload || null);
	});
...
function refreshTokens(refreshToken, strategy) {
	if (!refreshToken) {
		logger.debug("getRefreshToken didn't return a refresh-token");
