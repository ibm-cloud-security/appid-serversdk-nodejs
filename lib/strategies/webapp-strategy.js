...
WebAppStrategy.logout = function (req) {
	// register the logout to the auth server.
	// the correct access to the oauth url is via 'this.serviceConfig'.
	// However, the node.js sample calls WebAppStrategy.logout() in a a static way, so serviceConfig is not available directly.
	// we extract it here in a roundabout way:
	try {
		let serviceConfig = req._passport.instance._strategies['appid-webapp-strategy'].serviceConfig;
		let url = serviceConfig.getOAuthServerUrl();
		logAction(req, url, LOGOUT_ACTIVITY);
	}
	catch (error) {
		logger.debug('logging request skipped: ' + error);
	}
...
// this is the preferred non-static version:
...
function logAction(req, url, activity) {
	...
	if (!appIdAuthContext || !appIdAuthContext.accessToken || !appIdAuthContext.identityToken) {
		logger.debug("no session tokens found during logout");
		return;
	}
	...
	request({
		...
		if (error) {
			...
			//logger.debug('logging request ok: ' + response);
		}});
}
