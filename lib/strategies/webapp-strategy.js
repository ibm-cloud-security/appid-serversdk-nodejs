...
function WebAppStrategy(options = {}) {
	...
	ServiceUtil.loadConfig('WebAppStrategy', [
		constants.CLIENT_ID,
		constants.TENANT_ID,
		constants.SECRET,
		...
		.then(result => {
            ...
            console.log("SERVICE CONFIG WHEN INITIALIZED");
...
WebAppStrategy.prototype.logout = function (req) {
    this.serviceConfig.getOAuthServerUrl()
        .then(oauthServerUrl => {
            logAction(req, oauthServerUrl, LOGOUT_ACTIVITY);
            finishLogout(req);
        });
};
...
function handleChangePassword(req, options, strategy) {
	...
	generateChangePasswordUrl(userId, req, strategy)
		.then(changePasswordUrl => {
			...
			strategy.fail(new Error("Unable to generate change password url."));
		});
}
...
function handleForgotPassword(req, options, strategy) {
	...
	generateForgotPasswordUrl(req, strategy)
        .then(forgotPasswordUrl => {
            logger.debug("handleForgotPassword :: redirecting to", forgotPasswordUrl);
            strategy.redirect(forgotPasswordUrl);
        }).catch(error => {
			strategy.fail(new Error("Unable to generate forgot password  url."));
		});
}
...
function handleChangeDetails(req, options, strategy) {
    ...
    strategy.serviceConfig.getOAuthServerUrl()
        .then(oauthServerUrl => {
            var generateCodeUrl = oauthServerUrl + GENERATE_CODE_PATH;
            request({
                'url': generateCodeUrl,
                'auth': {
                ...
                if (!error) {
                    if (response.statusCode === 200) {
                        var code = body;
                        generateChangeDetailsUrl(code, req, strategy)
							.then(changeDetailsUrl => {
                                logger.debug("handleChangeDetails :: redirecting to", changeDetailsUrl);
                                req.session[WebAppStrategy.CLOUD_DIRECTORY_UPDATE_REQ] = true;
                                strategy.redirect(changeDetailsUrl);
							}).catch(error => {
								strategy.fail(new Error("Unable to generate change details url."));
							});
                    } else {
...
function redirectForAuthorization(req, options, strategy) {
	...
	generateAuthorizationUrl(options, req, strategy)
		.then(authUrl => {
...
function generateForgotPasswordUrl(req, strategy) {
    ...
    return serviceConfig.getOAuthServerUrl()
        .then(oauthServerUrl => {
            const forgotPasswordEndpoint = oauthServerUrl + FORGOT_PASSWORD_PATH;
...
function generateChangePasswordUrl(userId, req, strategy) {
    ...
    return serviceConfig.getOAuthServerUrl()
        .then(oauthServerUrl => {
            const changePasswordEndpoint = oauthServerUrl + CHANGE_PASSWORD_PATH;
            const redirectUri = serviceConfig.getRedirectUri();
...
function generateChangeDetailsUrl(code, req, strategy) {
    ...
    return serviceConfig.getOAuthServerUrl()
        .then(oauthServerUrl => {
            const changeDetailsEndpoint = oauthServerUrl + CHANGE_DETAILS_PATH;
            const redirectUri = serviceConfig.getRedirectUri();
...
function generateAuthorizationUrl(options, req, strategy) {
	const serviceConfig = strategy.serviceConfig;
    console.log("SERVICE CONFIG WHEN NEEDED");
    ...
    return serviceConfig.getOAuthServerUrl()
        .then(oauthServerUrl => {
            const authorizationEndpoint = oauthServerUrl + AUTHORIZATION_PATH;
            const redirectUri = serviceConfig.getRedirectUri();
...
function retrieveTokens(formData, strategy) {
	...
	return new Promise((resolve, reject) => {
        ...
        serviceConfig.getOAuthServerUrl()
            .then(oauthServerUrl => {
                const tokenEndpoint = oauthServerUrl + TOKEN_PATH;
                request({
                    method: "POST",
                    url: tokenEndpoint,
                    auth: {
                    ...
                    if (err) {
                        ...
                        Promise.all([
                            ...
                            TokenUtil.decodeAndValidate(identityTokenString)
