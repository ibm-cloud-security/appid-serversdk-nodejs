 ...
 * @param options.audience (optional): the resource URI. If not provided, then options.scope need to be the full (prefixed) scopes.
 *        Nevertheless, it is highly recommended you provide an audience parameter.
 * @returns {*}
 */
ApiStrategy.prototype.authenticate = function (req, options = {}) {
  ...
  var requiredScopes = ApiStrategy.DEFAULT_SCOPE;
  if (options.scope && options.scope.trim()) { // if the required scopes are just whitespace, skip.
	const scopesArray = options.scope.split(" ").filter(scope => scope !== ''); // split by spaces and keep only non empty scopes
  ...
  TokenUtil.decodeAndValidate(accessTokenString, this.serviceConfig.getOAuthServerUrl()).then(function (accessToken) {
	...
	if (options.audience && options.audience.trim() && accessToken.audience) {
		// audience is an array (currently we support only 1 item in the array)
		const trimmedRequiredAudience = options.audience.trim();
		const requiredList = trimmedRequiredAudience.split(' ');
		...
		const trimmedSuppliedAudience = accessToken.audience.trim();
		const suppliedList = trimmedSuppliedAudience.split(' ');
		if (suppliedList.length > 1) {
			return self.fail(buildWwwAuthenticateHeader("token multiple audiences are not supported", ERROR.INVALID_REQUEST), 400);
		}
		if (trimmedRequiredAudience !== trimmedSuppliedAudience) {
			return self.fail(buildWwwAuthenticateHeader("audience mismatch. expected:" + trimmedSuppliedAudience +
				" got:"+trimmedRequiredAudience, ERROR.INVALID_REQUEST), 400);
		}
	}
