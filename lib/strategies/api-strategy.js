 ...
 *
 * @param req
 * @param options: should include 'audience' (string) which is the application-URI and 'scope' (string) which is the required scopes, not prefixed.
 *                 can also include 'scope' (string) with prefixed scopes, without providing an 'audience' parameter.
 * @returns {*}
 */
ApiStrategy.prototype.authenticate = function (req, options = {}) {
  ...
  if (options.scope && options.scope.trim()) { // if the required scopes are just whitespace, skip.
	...
	const optionsScopes = options.scope.split(" ");
    for (const requiredScope of optionsScopes) {
	  requiredScopes += " " + prefix + requiredScope;
	}
  }
  ...
  TokenUtil.decodeAndValidate(accessTokenString, this.serviceConfig.getOAuthServerUrl()).then(function (accessToken) {
	...
	const requiredScopeElements = requiredScopes.split(" ");
	const suppliedScopeElements = accessToken.scope.split(" ");
	for (const requiredScope of requiredScopeElements) {
	  if (!suppliedScopeElements.includes(requiredScope)) {
