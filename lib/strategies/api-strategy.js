...
function ApiStrategy(options) {
    ...
    ServiceUtil.loadConfig('APIStrategy', [], options)
		.then(result => {
...
ApiStrategy.prototype.authenticate = function (req, options={}) {
	...
	TokenUtil.decodeAndValidate(accessTokenString).then(function (accessToken) {
        ...
        TokenUtil.validateIss(accessToken, self.serviceConfig)
			.then(() => {
				...
				if (authHeaderComponents.length === 3) {
					...
					TokenUtil.decodeAndValidate(identityTokenString).then(function (identityToken) {
						if (identityToken) {
							TokenUtil.validateIss(identityToken, self.serviceConfig)
								.then(() => {
									...
									return self.success(identityToken);
								}).catch(function (error) {
									logger.warn("Invalid iss in identity_token. Proceeding with access_token only");
                                	logger.debug("authentication success");
				...
				logger.debug("authentication failed due to invalid issuer field in access token");
				return self.fail(buildWwwAuthenticateHeader(requiredScope, ERROR.INVALID_TOKEN), 401);
			});
	}).catch(function () {
