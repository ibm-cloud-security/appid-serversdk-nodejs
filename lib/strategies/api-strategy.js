...
ApiStrategy.prototype.authenticate = function (req, options={}) {
	...
	if (options.scope) {
	    // if no options.audience is provided, requests will fail later
	    // because of insufficient_scope since we don't know the prefix.
	    // we write the required scopes without prefixes in this case, to send them with wwwAuthenticateHeader
	    const prefix = options.audience ? options.audience + "/" : "";
	    for (const required of options.scope.split(" ")) {
	        requiredScope += " " + prefix + required;
	    }
	}
	...
	TokenUtil.decodeAndValidate(accessTokenString, this.serviceConfig.getOAuthServerUrl()).then(function (accessToken) {
		...
		const requiredScopeElements = requiredScope.split(" ");
		const suppliedScopeElements = accessToken.scope.split(" ");
		for (const required of requiredScopeElements) {
		  if (!suppliedScopeElements.includes(required)) { // TODO: look for prefix/required for user provided scopes
