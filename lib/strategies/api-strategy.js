...
ApiStrategy.prototype.authenticate = function (req, options={}) {
	...
	if (options.scope) {
	    if (!options.audience) { // supplied scope but not audience
	        return self.fail(buildWwwAuthenticateHeader("", "received required scopes without audience"), 400);
	    }
	    for (const required of options.scope.split(" ")){
	        requiredScope += " " + options.audience + "/" + required;
	    }
	}
