...
ApiStrategy.prototype.authenticate = function (req, options={}) {
	...
	TokenUtil.decodeAndValidate(accessTokenString, PublicKeyUtil.getPublicKeysEndpoint(self.serviceConfig.getOAuthServerUrl())).then(function (accessToken) {
		if (!accessToken) {
		...
		if (authHeaderComponents.length === 3) {
			...
			TokenUtil.decodeAndValidate(identityTokenString, PublicKeyUtil.getPublicKeysEndpoint(self.serviceConfig.getOAuthServerUrl())).then(function (identityToken) {
				if (identityToken) {
