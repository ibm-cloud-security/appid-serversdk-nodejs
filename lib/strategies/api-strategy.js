...
function ApiStrategy(options) {
	...
	PublicKeyUtil.retrievePublicKeys(this.serviceConfig.getOAuthServerUrl());
}
    ...
    TokenUtil.decodeAndValidate(accessTokenString).then(function(accessToken) {
        ...
        if (authHeaderComponents.length === 3) {
            ...
            TokenUtil.decodeAndValidate(identityTokenString).then(function(identityToken) {
                ...
                return self.fail(buildWwwAuthenticateHeader(requiredScope, ERROR.INVALID_TOKEN), 401); //todo shiran - check if is the right err
            });
        } else {
