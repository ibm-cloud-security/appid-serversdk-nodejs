...
const APP_TO_APP_GRANT_TYPE = 'client_credentials';

function TokenManager(options) {
	logger.debug("Initializing token manager");
	const { CLIENT_ID, TENANT_ID, SECRET, OAUTH_SERVER_URL } = constants;
...
TokenManager.prototype.getApplicationIdentityToken = function () {
	...
	return getTokens(tokenEndPoint, clientId, secret, APP_TO_APP_GRANT_TYPE)
		.then((tokenResponse) => {
			if(!tokenResponse) {
				logger.error('Unable to parse token response');
				throw Error('Unable to parse token response');
		...
		request({
			...
			if (error) {
				...
				if (response.statusCode === 401 || response.statusCode === 403) {
					...
					logger.error('Failed to obtain token. Error: ', JSON.parse(body).error_description);
...
function validateToken(tokenType, token, serviceConfig) {
	...
	return TokenUtil.decodeAndValidate(token, serviceConfig).then((validatedToken) => {
		...
		} else if (!TokenUtil.validateIssAndAud(validatedToken, serviceConfig)) {
