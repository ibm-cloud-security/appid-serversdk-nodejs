...
function decode(tokenString) {
  ...
  if (tokenString === "bad_scope") {
    return {
      scope: "bad_scope"
    };
  }
  if (tokenString === "null_scope") {
    return null;
  }
  if (tokenString === "access_token_mock_test_scope") {
    return {
      scope: "test_scope"
    };
  }
  if (tokenString === "id_token_mock_test_scope") {
    return {
      scope: "test_scope"
    };
  }
...
function decodeAndValidate(tokenString) {
  ...
  if (tokenString === "invalid_token") {
    ...
    deferred.resolve({
      scope: "appid_default app/scope1 app/scope2 app/scope3"
    });
  } else {
    deferred.resolve({
      scope: "appid_default"
...
const switchIssuerState = () => {
  shouldSwitchIssuerState = true;
};
const setValidateIssAndAudResponse = (isValid) => {
  isIssuerAndAudValid = isValid;
};
const checkSwitch = () => {
